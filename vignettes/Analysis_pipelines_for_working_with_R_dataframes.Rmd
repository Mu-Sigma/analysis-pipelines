...
This package contains 4 vignettes:
...
You can register your own *data* or *non-data* functions by calling the `registerFunction` method. This adds the user-defined function to the object's registry. 
...
In order to register a function, first the function must be defined in the Global environment, before calling the `registerFunction` method.
...
obj2 %>>% registerFunction("bivariatePlots", heading = "Bivariate Analysis") -> obj2
```

This user-defined function can now be registered as part of an *AnalysisPipeline* object. After registration, the registry in the object will be updated with the user-defined function. 
...
obj %>>% registerFunction("getColor", isDataFunction = F, firstArgClass = "character") -> obj
# obj %>>% registerFunction("getColor", heading = "Get Color", isDataFunction = F, firstArgClass = "numeric") -> obj

obj %>>% registerFunction("getColumnName", isDataFunction = F, firstArgClass = "character") -> obj
...
Pipelines can also be saved and loaded for re-use and execution on new data. The pipeline can be saved by calling the `savePipeline` function.
...
sampleExceptionObj <- obj2 %>>% registerFunction('sampleFunction', "Sample", exceptionFunction = "sampleException")

sampleExceptionObj %>>% getRegistry
```
