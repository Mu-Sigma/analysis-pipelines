...
# Define pipeline 
# Do data prep
pipelineObj %>% castKafkaStreamAsString() %>% convertKafkaValueFromJson(schema = consumerDataSchema, outAsIn = T) %>% convertStructToDf(outAsIn = T) %>% castDfColumns(outAsIn = T, storeOutput = T) -> pipelineObj
...
Currently, streaming pipelines have the limitation that they are able to execute only linear flows, as this constrained by *Apache Spark Structured Streaming.* Non-linear flows can be defined but might throw execution errors in runtime.
