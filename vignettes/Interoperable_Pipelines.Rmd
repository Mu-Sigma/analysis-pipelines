...
registerFunction(functionName = "sparkFilterData", engine = "spark") 
...
registerFunction("getTargetForPyClassification", isDataFunction = F, firstArgClass = "data.frame", engine = "r")
registerFunction("getFeaturesForPyClassification", isDataFunction = F, firstArgClass = "data.frame", engine = "r")
```
...
```{r}
pipelineObj %>>% sparkFilterData("Class == 'good'") %>>% 
  sparkFilterData("Occupancy > 0.7", outAsIn = T) %>>%
  rBivarPlots(select_var_name_1 = "Compet_Occupancy", select_var_name_2 =  "Occupancy", 
                     priColor = "blue", secColor = "green", outAsIn = T, storeOutput = T) %>>%
  getFeaturesForPyClassification(dataset = ~f2, featureNames = c("direct_call_booking_pct",
                                                                 "loyalty_pct",
                                                                 "call_center_booking_pct")) %>>% 
          getTargetForPyClassification(dataset = ~f2, targetVarName = "Class", positiveClass = "good") %>>%
          trainDecisionTree_py(data = ~f4, target = ~f5, storeOutput = T) -> twoFilterPipeline
