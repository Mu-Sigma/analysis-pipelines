...
pipelineObj %>>% registerFunction(functionName = "sparkFilterData", engine = "spark", outAsIn = T) -> pipelineObj
...
pipelineObj %>>% registerFunction(functionName = "rBivarPlots", engine = "r", outAsIn = T) -> pipelineObj
...
pipelineObj %>>% rBivarPlots(select_var_name_1 = "Compet_Occupancy", select_var_name_2 =  "Occupancy", 
                     priColor = "blue", secColor = "green") -> vizWithoutFilterPipeline
...
pipelineObj %>>% sparkFilterData("Class == 'good'") %>>% 
  rBivarPlots(select_var_name_1 = "Compet_Occupancy", select_var_name_2 =  "Occupancy", 
                     priColor = "blue", secColor = "green") -> singleFilterPipeline
singleFilterPipeline %>>% assessEngineSetUp
...
pipelineObj %>>% sparkFilterData("Class == 'good'") %>>% 
  ...
  rBivarPlots(select_var_name_1 = "Compet_Occupancy", select_var_name_2 =  "Occupancy", 
                     priColor = "blue", secColor = "green") -> twoFilterPipeline
twoFilterPipeline %>>% assessEngineSetUp
twoFilterPipeline %>>% generateOutput -> opWith2Filters
opWith2Filters %>>% getOuputByOrderId(3)
```
...
```{r}
twoFilterPipeline %>>% savePipeline(RDSPath = "~/Downloads/twoFilterPipeline.Rda")
```
...
```{r}
pipelineObjLoaded <- loadPipeline(RDSPath = "~/Downloads/testing1.Rda", 
                                  input = SparkR::as.data.frame(inputDataset)[1:100,])
